...
|---|---|---|---|
| **CRAWL / CHECK** | Survey all tracks for blockers, stale context, ownership, wire health, and cross-track collisions | Blocker list + stale-context report | Operator |
...
7. **Productization timeline assumes operator availability.** All release dates assume the operator has uninterrupted focus. This has never been true.
...
| **Research (conference submission) + Brand (artpack release)** | Track 3 + Track 2 | **Low** | No shared code or time. But both compete for operator attention (the operator's review bandwidth). |
